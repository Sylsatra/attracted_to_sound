        ...
        try {
            if (!QuantifiedOptionalBridge.isAvailable()) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    SoundAttractMod.LOGGER.debug("[Quantified] Quantified API 2.1 bridge unavailable; falling back to local workers/cache.");
                }
                return;
            }

            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.debug("[Quantified] Successfully initialized with Quantified API 2.1");
            }
        } catch (Throwable t) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
