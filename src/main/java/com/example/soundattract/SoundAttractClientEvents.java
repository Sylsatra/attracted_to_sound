...
public class SoundAttractClientEvents {
    ...
    public static void onPlaySoundEvent(PlaySoundEvent event) {
        ...
        if (event.getSound() instanceof AbstractSoundInstance soundInstance) {
            ...
            boolean isStepSound = se != null && se.getLocation().getPath().contains("step") && clientPlayer.position().distanceToSqr(x, y, z) < 1.5 * 1.5;
            if (isStepSound) {
            ...
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("Sending sound to server: {} at ({}, {}, {}) with range {} in dimension {}", soundRL, x, y, z, calculatedRange, dim);
            }
