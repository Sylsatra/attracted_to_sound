...
public class FollowerEdgeRelayGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (this.mob.getTarget() != null && this.mob.getTarget().isAlive()) {
            ...
            if (threshold > 0 && newSound.weight >= threshold && isEligibleForOverride()) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] Mob {} dropping target for high-weight sound ({}).", this.mob.getName().getString(), newSound.weight);
                }
        ...
        this.raidScheduled = false;
        if (SoundAttractConfig.COMMON.debugLogging.get()) {
            Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] START GOING_TO_SOUND: mob={} leader={} edgeMob={} deserter={} soundPos={} weight={}",
    ...
    public boolean canContinueToUse() {
        ...
        if (!isMobEligible() || this.mob.isVehicle() || this.mob.isSleeping()) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] canContinueToUse -> FALSE (suppressed or ineligible) for mob {}.", mob.getName().getString());
            }
            return false;
        }
        if (this.targetSoundPos == null) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] canContinueToUse -> FALSE (targetSoundPos null) for mob {}.", mob.getName().getString());
            }
            return false;
        }
        boolean smartEdge = SoundAttractConfig.COMMON.edgeMobSmartBehavior.get();
        ...
        if (leader != mob && smartEdge && mob.position().distanceToSqr(Vec3.atCenterOf(targetSoundPos)) < getArrivalDistance() * getArrivalDistance()) {
            ...
            if (isDeserterHere) {
                ...
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] Arrived at sound; deserter will NOT return. Ending goal for mob {}.", mob.getName().getString());
                }
                return false;
            }

            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] Arrived at sound; holding to return for mob {}.", mob.getName().getString());
            }
            return true;
        }
        ...
        if (bestSoundNow == null) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] canContinueToUse -> FALSE (no best sound now) for mob {}.", mob.getName().getString());
            }
            return false;
        }
        ...
        if (bestSoundNow.weight > this.currentTargetWeight * switchRatio) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] canContinueToUse -> FALSE (switching to stronger sound {} > {}) for mob {}.",
    ...
    public void stop() {
        ...
        if (leader != mob && smartEdge && edgeMobState == EdgeMobState.GOING_TO_SOUND) {
            ...
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] stop(): follower {} transitioning to RETURNING_TO_LEADER.", mob.getName().getString());
            }
            edgeArrivalTicks = 0;
        } else {
            edgeMobState = null;
        ...
        if (SoundAttractConfig.COMMON.debugLogging.get()) {
            Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] stop(): mob {} cleanup complete (state reset).", mob.getName().getString());
        }
    }
    ...
    public void tick() {
        ...
        if (this.blockBreakerGoal != null && ((MobAccessor) this.mob).getGoalSelector().getAvailableGoals().stream().anyMatch(g -> g.getGoal() == this.blockBreakerGoal)) {
            ...
            if (bestPossibleSound != null && this.cachedSound != null && !areSoundsEffectivelySame(bestPossibleSound, this.cachedSound)) {
                ...
                if (bestPossibleSound.weight > this.cachedSound.weight * switchRatio) {
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} found a better sound ({} > {}). Stopping block breaking.", this.mob.getName().getString(), bestPossibleSound.weight, this.cachedSound.weight);
                    }
        ...
        if (stuckTicks >= 40 && SoundAttractConfig.COMMON.enableBlockBreaking.get()) {
            if (SoundAttractConfig.COMMON.debugLogging.get() && stuckTicks == 40) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal DEBUG] {} truly stuck. isStuck(): {}", mob.getName().getString(), this.mob.getNavigation().isStuck());
            }
            if (this.blockBreakerGoal == null) {
                            ...
                            if (this.blockBreakerGoal == null) {
                                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                                    if (this.edgeMobState == EdgeMobState.RETURNING_TO_LEADER && destLeader != null) {
                                        Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} returning to leader {} but stuck. Deploying BlockBreakerPosGoal toward leader.", mob.getName().getString(), destLeader.getName().getString());
                                    } else {
                                        Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} navigation DONE and far from goal. Deploying BlockBreakerPosGoal.", mob.getName().getString());
                                    }
                                }
        ...
        if (this.edgeMobState != EdgeMobState.RETURNING_TO_LEADER && (!smartEdge || (leader == mob && !isDeserter))) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowerEdgeRelayGoal DEBUG] Fallback direct move to sound for {} (leader==mob: {}, smartEdge: {}, deserter: {}).", mob.getName().getString(), leader == mob, smartEdge, isDeserter);
            }
        ...
        if (edgeMobState == EdgeMobState.GOING_TO_SOUND) {
                ...
                if (edgeArrivalTicks >= EDGE_WAIT_TICKS || foundPlayerOrHit) {
                    ...
                    if (foundPlayerOrHit && !MobGroupManager.isDeserter(this.mob)) {
                        ...
                        if (raidLeader != null && !raidScheduled
                            ...
                            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                                if (usedFallbackArr) {
                                    Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} scheduled RAID for nearest leader {} at {} (arrival-phase, no immediate relay).",
                                            mob.getName().getString(), raidLeader.getName().getString(), this.targetSoundPos);
                                } else {
                                    Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} scheduled RAID for leader {} at {} (arrival-phase, no immediate relay).",
                                            mob.getName().getString(), raidLeader.getName().getString(), this.targetSoundPos);
                                }
                            }
                        } else if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} arrival-phase but no suitable leader found to schedule RAID.", mob.getName().getString());
                        }
                    }

                    if (MobGroupManager.isDeserter(this.mob)) {
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} is a deserter; will NOT return after arrival. Ending goal.", mob.getName().getString());
                        }
                        targetSoundPos = null;
                        edgeMobState = null;
                        cachedReturnLeader = null;
            ...
            if (MobGroupManager.isDeserter(this.mob)) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowerEdgeRelayGoal] {} is a deserter but entered RETURNING_TO_LEADER; aborting return.", mob.getName().getString());
                }
                if (this.mob.isSprinting()) this.mob.setSprinting(false);
                targetSoundPos = null;
                edgeMobState = null;
