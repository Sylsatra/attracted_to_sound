...
public class AttractionGoal extends Goal {
        ...
        if (SoundAttractConfig.COMMON.enableStealthMechanics.get()) {
            for (net.minecraft.world.item.ItemStack armorItem : player.getArmorSlots()) {
                if (!armorItem.isEmpty()) {
                    String itemId = net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(armorItem.getItem()).toString();
    ...
    private boolean shouldSuppressTargeting() {
        return SoundAttractConfig.COMMON.enableStealthMechanics.get() && com.example.soundattract.StealthDetectionEvents.shouldSuppressTargeting(this.mob);
    }
    ...
    public boolean canUse() {
        ...
        if (leader != mob && smartEdge) {
            ...
            if (!hasDirectSound && !hasRelayedSound) {
                SoundAttractMod.LOGGER.info("AttractionGoal canUse: Non-leader " + mob.getName().getString() + " has no direct or relayed sound in smart edge mode.");
                return false;
            }
        }
        ...
        if (SoundAttractConfig.COMMON.debugLogging.get()) {
            com.example.soundattract.SoundAttractMod.LOGGER.info("[AttractionGoal] Mob {} found sound: pos={}, range={}, weight={}", mob.getName().getString(), newSound.pos, newSound.range, newSound.weight);
        }
    ...
    public boolean canContinueToUse() {
       ...
       Mob leader       = MobGroupManager.getLeader(mob);
       if (leader != mob && smartEdge &&
           mob.position().distanceToSqr(Vec3.atCenterOf(targetSoundPos)) < getArrivalDistance()*getArrivalDistance())
           return false;

       SoundTracker.SoundRecord live =
           SoundTracker.findNearestSound(mob, mob.level(), mob.blockPosition(), mob.getEyePosition());

       if (live == null || !live.pos.equals(targetSoundPos)) {      
           if (SoundAttractConfig.COMMON.debugLogging.get())
               SoundAttractMod.LOGGER.info(
                "[AttractionGoal] canContinueToUse: live sound vanished for {} (target {})",
                mob.getName().getString(), targetSoundPos);
           return false;
       }
    ...
    public void stop() {
        ...
        if (leader != mob && smartEdge && edgeMobState == EdgeMobState.GOING_TO_SOUND) {
            ...
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " transitioning to RETURNING_TO_LEADER.");
            }
            edgeArrivalTicks = 0;
        } else { 
            edgeMobState = null;
    ...
    public void tick() {
        if (targetSoundPos == null) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                 SoundAttractMod.LOGGER.info("AttractionGoal tick: targetSoundPos is null for " + mob.getName().getString() + ", goal will likely stop.");
            }
            return; 
        }
        ...
        if (lastPos != null && lastPos.equals(mob.blockPosition())) {
            stuckTicks++;
            if (stuckTicks >= RECALC_THRESHOLD && !this.mob.getNavigation().isStuck()) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    SoundAttractMod.LOGGER.info("AttractionGoal tick: " + mob.getName().getString() + " stuck for " + stuckTicks + " ticks at " + lastPos + ", recalculating path to " + targetSoundPos);
                }
        ...
        if (currentPursuedSound == null || !currentPursuedSound.pos.equals(this.targetSoundPos)) {
             if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("AttractionGoal tick: Cached sound mismatch for {} during expiration check. Target: {}, Cached: {}", mob.getName().getString(), targetSoundPos, (currentPursuedSound != null ? currentPursuedSound.pos : "null"));
            }
        ...
        if (currentPursuedSound == null || currentPursuedSound.ticksRemaining <= 0) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("AttractionGoal tick: Sound at " + targetSoundPos + " (from cache) expired or removed for " + mob.getName().getString());
            }
            targetSoundPos = null;
            return;
        }
        ...
        if (leader == mob) {
            ...
            if (relays != null && !relays.isEmpty()) {
                for (MobGroupManager.SoundRelay relay : relays) {
                    SoundEvent relayEvent = ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.parse(relay.soundId));
SoundTracker.SoundRecord relayedSoundRecord = new SoundTracker.SoundRecord(relayEvent, new BlockPos((int)relay.x, (int)relay.y, (int)relay.z), 20, mob.level().dimension().location().toString(), relay.range, relay.weight); // Assuming default ticks for relayed sound
                    if (cachedSound == null || (relayedSoundRecord.weight > cachedSound.weight * SoundAttractConfig.COMMON.soundSwitchRatio.get() ||
                        (Math.abs(relayedSoundRecord.weight - cachedSound.weight) < 0.001 && relayedSoundRecord.pos.distSqr(mob.blockPosition()) < cachedSound.pos.distSqr(mob.blockPosition())))) { // Compare distance to mob
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("Leader " + mob.getName().getString() + " switching to relayed sound (ID: " + relay.soundId + ") from follower at " + relayedSoundRecord.pos + " (w:" + relayedSoundRecord.weight + ", r:" + relayedSoundRecord.range + ")");
                        }
                        cachedSound = relayedSoundRecord;
                        targetSoundPos = cachedSound.pos;
                        currentTargetWeight = cachedSound.weight;
                        this.mob.getNavigation().moveTo(targetSoundPos.getX(), targetSoundPos.getY(), targetSoundPos.getZ(), this.moveSpeed);
                        this.pursuingSoundTicksRemaining = relayedSoundRecord.ticksRemaining > 0 ? relayedSoundRecord.ticksRemaining : DynamicScanCooldownManager.currentScanCooldownTicks; // Update pursuit timer
                    }
                }
            }
        }
        else if (leader != null && smartEdge) {
            if (edgeMobState == null) {
                edgeMobState = EdgeMobState.GOING_TO_SOUND;
                this.mob.getNavigation().moveTo(targetSoundPos.getX(), targetSoundPos.getY(), targetSoundPos.getZ(), this.moveSpeed);
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " (leader: " + leader.getName().getString() + ") starting to move to sound at " + targetSoundPos + " (EdgeMobState: GOING_TO_SOUND)");
                }
            }

            if (edgeMobState == EdgeMobState.GOING_TO_SOUND) {
                this.mob.getNavigation().moveTo(targetSoundPos.getX(), targetSoundPos.getY(), targetSoundPos.getZ(), this.moveSpeed);
                if (this.mob.position().distanceToSqr(Vec3.atCenterOf(targetSoundPos)) < getArrivalDistance() * getArrivalDistance()) {
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " arrived at sound location " + targetSoundPos + ". Waiting for " + EDGE_WAIT_TICKS + " ticks.");
                    }
                    edgeArrivalTicks++;
                    if (edgeArrivalTicks >= EDGE_WAIT_TICKS || foundPlayerOrHit) {
                        if (!relayedToLeader && foundPlayerOrHit) {
                            ...
                            if (soundToRelay != null && soundToRelay.pos.equals(targetSoundPos) && soundToRelay.ticksRemaining > 0 && soundToRelay.soundId != null) {
                                ...
                                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                                    SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " relayed 'foundPlayerOrHit' event (Sound ID: " + soundToRelay.soundId + ") to leader " + leader.getName().getString() + " for sound at " + targetSoundPos);
                                }
                    ...
                    if (targetPlayer instanceof net.minecraft.world.entity.player.Player && targetPlayer.distanceToSqr(this.mob) < getDetectionRangeForPlayer(targetPlayer) * getDetectionRangeForPlayer(targetPlayer)) {
                        foundPlayerOrHit = true;
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " detected player " + targetPlayer.getName().getString() + " en route to sound.");
                        }
                    }
                }
                ...
                if (leader != null && !leader.isRemoved() && !leader.isDeadOrDying()) {
                    ...
                    if (this.mob.distanceToSqr(leader) < (getArrivalDistance() + 2.0) * (getArrivalDistance() + 2.0)) {
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " reached leader " + leader.getName().getString() + ". Stopping attraction goal (will re-evaluate).");
                        }
                        targetSoundPos = null;
                        edgeMobState = null; 
                    }
                    ...
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        SoundAttractMod.LOGGER.info("Follower " + mob.getName().getString() + " lost its leader while returning. Stopping attraction goal.");
                    }
                    targetSoundPos = null;
                    edgeMobState = null;
                }
        ...
        if (isPursuingSound && this.cachedSound != null) {
            if (pursuingSoundTicksRemaining > 0) {
                 ...
                 if (pursuingSoundTicksRemaining <= 0) {
                     ...
                     if (SoundAttractConfig.COMMON.debugLogging.get()) {
                         SoundAttractMod.LOGGER.info("AttractionGoal tick: " + mob.getName().getString() + " pursuit timer expired. Will re-evaluate sounds.");
                     }
                 }
            }
        ...
        if (this.mob.position().distanceToSqr(Vec3.atCenterOf(targetSoundPos)) < getArrivalDistance() * getArrivalDistance()) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("AttractionGoal tick: " + mob.getName().getString() + " arrived at " + targetSoundPos + ". Scan cooldown will apply if it re-evaluates canUse.");
            }
        }
    }
    ...
    protected SoundTracker.SoundRecord findInterestingSoundRecord() {
        ...
        SoundTracker.SoundRecord bestSoundOverall = SoundTracker.findNearestSound(this.mob, level, mobPos, mobEyePos);
            ...
            if (relays != null) {
                for (MobGroupManager.SoundRelay relay : relays) {
                    ...
                    if (bestSoundOverall == null || 
                        ...
                        (Math.abs(relayedSound.weight - bestSoundOverall.weight) < 0.001 && relayedSound.range > bestSoundOverall.range)) {
        ...
        if (currentTargetSound != null && bestSoundOverall != null && !areSoundsEffectivelySame(currentTargetSound, bestSoundOverall)) {
            ...
            boolean canSwitch = bestSoundOverall.weight > currentTargetSound.weight * switchRatio ||
                                (Math.abs(bestSoundOverall.weight - currentTargetSound.weight) < 0.001 && bestSoundOverall.pos.distSqr(mobPos) < currentTargetSound.pos.distSqr(mobPos));
        ...
        if (bestSoundOverall != null) {
            ...
            this.pursuingSoundTicksRemaining = com.example.soundattract.DynamicScanCooldownManager.currentScanCooldownTicks; 
        } else {
            if (leader == this.mob) {
                 this.isPursuingSound = false;
                 this.pursuingSoundTicksRemaining = 0;
