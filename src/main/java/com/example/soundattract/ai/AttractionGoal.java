...
public class AttractionGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        ...
        if (this.mob.getNavigation().isDone()) {
            ...
            if (leader == this.mob && this.chosenDest != null) {
                destination = new BlockPos(this.chosenDest);
            } else {
    ...
    public void stop() {
        ...
        if (breakingScheduled) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("[AttractionGoal] {} leaving BlockBreakerPosGoal running after stop", mob.getName().getString());
            }
            breakingScheduled = false;
            scheduledBreakerGoal = null;
            lastBreakerDest = null;
        }

        try {
            EnhancedAICompat.clearDigging(this.mob);
    ...
    public void tick() {
        ...
        if (fresh != null) {
            if (this.targetSoundPos != null && fresh.pos.equals(this.targetSoundPos)) {
                ...
                if (isBetterByWeight || isTieButCloser) {
                    ...
                    if (breakingScheduled && SoundAttractConfig.COMMON.enableBlockBreaking.get()) {
                        try {
                            ...
                            BlockBreakerPosGoal newGoal = new BlockBreakerPosGoal(this.mob, this.targetSoundPos, mult, toolOnly, properOnly, properOnly);
                            scheduledBreakerGoal = newGoal;
                            BlockBreakerManager.scheduleAdd(this.mob, newGoal, 1);
                            lastBreakerDest = this.targetSoundPos;
                            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                                SoundAttractMod.LOGGER.info("[AttractionGoal] {} rescheduled BlockBreakerPosGoal towards new target {}", mob.getName().getString(), this.targetSoundPos);
                            }
                        } catch (Throwable t) {
                            if (SoundAttractConfig.COMMON.debugLogging.get()) {
        ...
        double movedSqr = 0.0;
        if (lastPos != null) {
            movedSqr = mob.position().distanceToSqr(Vec3.atCenterOf(lastPos));
        }
        boolean incremented = false;
        if (lastPos != null && movedSqr < 1.0) {
            stuckTicks++;
            incremented = true;
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                if (stuckTicks == 1 || stuckTicks == 5 || stuckTicks == STUCK_THRESHOLD || (stuckTicks % 10 == 0)) {
                    SoundAttractMod.LOGGER.info("[AttractionGoal DEBUG] {} stuckTicks={}, movedSqr={}", mob.getName().getString(), stuckTicks, String.format("%.3f", movedSqr));
                }
            }
        } else {
            if (stuckTicks > 0 && SoundAttractConfig.COMMON.debugLogging.get()) {
                SoundAttractMod.LOGGER.info("[AttractionGoal DEBUG] {} unstuck reset. prev stuckTicks={}, movedSqr={}", mob.getName().getString(), stuckTicks, String.format("%.3f", movedSqr));
            }
            stuckTicks = 0;
        }

        if (stuckTicks == 0 || !incremented) {
            lastPos = mob.blockPosition();
        }


        boolean blockBreakingEnabled = SoundAttractConfig.COMMON.enableBlockBreaking.get();
        if (blockBreakingEnabled && this.targetSoundPos != null) {
            try {
                int maxY = EnhancedAICompat.getMaxY();
                boolean withinY = this.targetSoundPos.getY() <= maxY;
                ...
                double distToDestSqr = destination != null ? this.mob.blockPosition().distSqr(destination) : -1.0;

                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    SoundAttractMod.LOGGER.info("[AttractionGoal DEBUG] {} consider breaker: stuckTicks={}, withinY={}, targetY={}, maxY={}, navDone={}, distToDestSqr={}",
                            mob.getName().getString(), stuckTicks, withinY, this.targetSoundPos.getY(), maxY, navDone, String.format("%.3f", distToDestSqr));
                }

                if (this.stuckTicks >= STUCK_THRESHOLD && withinY) {
                    double mult = SoundAttractConfig.COMMON.blockBreakingTimeMultiplier.get();
                    if (mult < 0) {
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("[AttractionGoal] blockBreakingTimeMultiplier=-1; EnhancedAI time not accessible on 1.19.2, defaulting to 1.5");
                    ...
                    boolean toolOnly = SoundAttractConfig.COMMON.blockBreakingToolOnly.get();
                    boolean properOnly = SoundAttractConfig.COMMON.blockBreakingProperToolOnly.get();
                    if (!breakingScheduled || lastBreakerDest == null || !lastBreakerDest.equals(this.targetSoundPos)) {
                        BlockBreakerPosGoal goal = new BlockBreakerPosGoal(this.mob, this.targetSoundPos, mult, toolOnly, properOnly, properOnly);
                        scheduledBreakerGoal = goal;
                        BlockBreakerManager.scheduleAdd(this.mob, goal, 1);
                        lastBreakerDest = this.targetSoundPos;
                        if (!breakingScheduled && SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("[AttractionGoal] {} scheduled BlockBreakerPosGoal towards {} (priority 1)", mob.getName().getString(), this.targetSoundPos);
                        } else if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            SoundAttractMod.LOGGER.info("[AttractionGoal] {} updating BlockBreakerPosGoal towards new dest {} (priority 1)", mob.getName().getString(), this.targetSoundPos);
                        }
                        breakingScheduled = true;
                    }
                } else if (breakingScheduled && !withinY) {

                    if (scheduledBreakerGoal != null) {
                        BlockBreakerManager.scheduleRemove(this.mob, scheduledBreakerGoal);
                    }
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        SoundAttractMod.LOGGER.info("[AttractionGoal] {} scheduled removal of BlockBreakerPosGoal (destination Y not eligible)", mob.getName().getString());
                    }
                    breakingScheduled = false;
                    scheduledBreakerGoal = null;
                    lastBreakerDest = null;
    ...
    private void handleLeaderTick() {
        ...
        if (relays != null && !relays.isEmpty()) {
            for (MobGroupManager.SoundRelay relay : relays) {
                SoundEvent relayEvent = ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.tryParse(relay.soundId));
                SoundTracker.SoundRecord relayedSoundRecord = new SoundTracker.SoundRecord(
                        relayEvent, relay.soundId, new BlockPos((int) relay.x, (int) relay.y, (int) relay.z), 20,
                        mob.level.dimension().location().toString(), relay.range, relay.weight);

                if (cachedSound == null || relayedSoundRecord.weight > cachedSound.weight * SoundAttractConfig.COMMON.soundSwitchRatio.get()) {
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        SoundAttractMod.LOGGER.info("Leader " + mob.getName().getString() + " switching to relayed sound (ID: " + relay.soundId + ") from follower at " + relayedSoundRecord.pos);
                    }
        ...
        if (edgeMobState == EdgeMobState.GOING_TO_SOUND) {
            ...
            if (mob.position().distanceToSqr(Vec3.atCenterOf(targetSoundPos)) < getArrivalDistance() * getArrivalDistance()) {
                ...
                if (edgeArrivalTicks >= EDGE_WAIT_TICKS || foundPlayerOrHit) {
                    if (!relayedToLeader && foundPlayerOrHit && this.cachedSound != null) {
                        MobGroupManager.relaySoundToLeader(this.mob, this.cachedSound.soundId, this.cachedSound.pos.getX(), this.cachedSound.pos.getY(), this.cachedSound.pos.getZ(), this.cachedSound.range, this.cachedSound.weight, this.mob.level.getGameTime());
