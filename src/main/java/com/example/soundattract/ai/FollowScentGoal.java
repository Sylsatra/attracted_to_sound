...
public class FollowScentGoal extends Goal {
    ...
    public FollowScentGoal(PathfinderMob mob) {
        this.mob = mob;
        this.scentManager = ScentManager.getForLevel(mob.level());
    ...
    private void handleStuckMob() {
        if (SoundAttractConfig.COMMON.enableBlockBreaking.get() && this.blockBreakerGoal == null) {
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowScentGoal] Mob {} is stuck moving to scent. Deploying BlockBreakerPosGoal.", mob.getName().getString());
            }
        ...
        if (stuckTicks > 200) {
            ...
            if (freshestNode != null && freshestNode.getPosition().distanceToSqr(mob.position()) < 400.0) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowScentGoal] Mob {} stuck for >200 ticks near freshest node. Triggering ambush from outside.", mob.getName().getString());
                }
                startAmbush();
                this.targetPos = null;
            } else {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowScentGoal] Mob {} stuck for >200 ticks mid-trail. Canceling scent tracking.", mob.getName().getString());
                }
                this.targetPos = null;
            }
            if (this.blockBreakerGoal != null) {
            ...
            if (smartEdge) {
                if (MobGroupManager.isEdgeMob(mob)) {
                    ...
                    if (leader != null && !RaidManager.isRaidTicking(leader) && !RaidManager.isRaidAdvancing(leader)) {
                        ...
                        if (SoundAttractConfig.COMMON.debugLogging.get()) {
                            Soundattract.LOGGER.info("[FollowScentGoal] Edge mob {} triggered scent raid for leader {} at {}",
                ...
                if (leader != null && !RaidManager.isRaidTicking(leader) && !RaidManager.isRaidAdvancing(leader)) {
                    RaidManager.scheduleRaid(leader, raidTarget, now);
                    if (SoundAttractConfig.COMMON.debugLogging.get()) {
                        Soundattract.LOGGER.info("[FollowScentGoal] Mob {} triggered scent raid via nearest leader {} at {}",
                                mob.getName().getString(), leader.getName().getString(), raidTarget);
                    }
                }
            }
    ...
    private boolean findScentTrail() {
        ...
        if (targetScentOwner == null) {
            ...
            if (SoundAttractConfig.COMMON.debugLogging.get()) {
                Soundattract.LOGGER.info("[FollowScentGoal] Mob {} acquired INITIAL scent trail for owner {} at {}.", 
            ...
            if (nextNode != null) {
                if (SoundAttractConfig.COMMON.debugLogging.get()) {
                    Soundattract.LOGGER.info("[FollowScentGoal] Mob {} found NEXT scent node at {} (time: {}).", 
