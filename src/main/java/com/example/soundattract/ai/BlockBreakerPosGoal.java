...
public class BlockBreakerPosGoal extends Goal {
    ...
    public boolean canStart() {
        ...
        if (!cfg.enableBlockBreaking) return false;
        if (destinationPos == null) return false;
        if (toolOnly && !isHoldingTool()) return false;
        this.targetPos = findFirstBlockingBlock(miner.getWorld(), miner, destinationPos);
        ...
        this.targetState = miner.getWorld().getBlockState(this.targetPos);
    ...
    public void start() {
        if (targetPos != null) {
            ...
            double distSq = miner.getPos().squaredDistanceTo(Vec3d.ofCenter(targetPos));
    ...
    public void stop() {
        if (targetPos != null) {
            if (miner.getWorld() instanceof ServerWorld sw) {
    ...
    public void tick() {
        ...
        double distSqToTarget = miner.getPos().squaredDistanceTo(Vec3d.ofCenter(targetPos));
    ...
    private void initBlockBreak() {
        this.targetState = miner.getWorld().getBlockState(targetPos);
    ...
    private int computeTicksToBreak(BlockState state) {
        float hardness = state.getHardness(miner.getWorld(), targetPos);
