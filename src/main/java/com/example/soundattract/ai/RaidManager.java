...
/**
 * RAID orchestration per dimension.
 * Edge mobs can schedule a RAID for their leader at a target position.
 * - Ticking: countdown until advancing begins (followers rally to leader).
 * - Advancing: leader/followers move toward the recorded target position.
 */
public class RaidManager {
    ...
    private static final Map<Identifier, PerWorldData> DATA = new ConcurrentHashMap<>();

    private static PerWorldData getData(ServerLevel level) {
        return DATA.computeIfAbsent(SoundTracker.getDimensionKey(level), k -> new PerWorldData());
    }
