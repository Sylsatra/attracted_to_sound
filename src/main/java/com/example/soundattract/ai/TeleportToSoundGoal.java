...
/**
 * Teleport helper goal: teleports an ally mob to the current tracked sound.
 *
 * Backported from the 1.21.1 NeoForge implementation to 1.19.2 Forge.
 */
public class TeleportToSoundGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (teleporterTagStr == null || teleporterTagStr.isEmpty()) return false;
        ResourceLocation teleporterRl = ResourceLocation.tryParse(teleporterTagStr);
        if (teleporterRl == null) return false;
        TagKey<EntityType<?>> teleporterTag = TagKey.create(Registry.ENTITY_TYPE_REGISTRY, teleporterRl);
        ...
        this.targetSound = SoundTracker.findNearestSound(this.mob, this.mob.level, this.mob.blockPosition(), this.mob.getEyePosition(), null);
        if (this.targetSound == null) return false;

        String targetTagStr = SoundAttractConfig.COMMON.teleportCanBeTeleportedTag.get();
        if (targetTagStr == null || targetTagStr.isEmpty()) return false;
        ResourceLocation targetRl = ResourceLocation.tryParse(targetTagStr);
        if (targetRl == null) return false;
        TagKey<EntityType<?>> targetTag = TagKey.create(Registry.ENTITY_TYPE_REGISTRY, targetRl);
    ...
    private void applySpeedFreeze(LivingEntity entity) {
        if (!entity.hasEffect(MobEffects.MOVEMENT_SLOWDOWN)
                || entity.getEffect(MobEffects.MOVEMENT_SLOWDOWN).getAmplifier() < 10) {
    ...
    private void teleportSafely(LivingEntity entity, double pX, double pY, double pZ) {
        ...
        boolean blocksMotion = mat != null && mat.blocksMotion();
