...
public class TeleportToSoundGoal extends Goal {
    private static final Identifier SPEED_FREEZE_ID = Identifier.of("soundattract", "teleport_to_sound_freeze");
    ...
    public boolean canStart() {
        ...
        TagKey<EntityType<?>> teleporterTag = TagKey.of(RegistryKeys.ENTITY_TYPE, Identifier.of(teleporterTagStr));
        ...
        TagKey<EntityType<?>> targetTag = TagKey.of(RegistryKeys.ENTITY_TYPE, Identifier.of(targetTagStr));
    ...
    private void applySpeedFreeze(LivingEntity entity) {
        ...
        if (inst != null && inst.getModifier(SPEED_FREEZE_ID) == null) {
            inst.addTemporaryModifier(new EntityAttributeModifier(SPEED_FREEZE_ID, -1.0, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));
        }
    }
    ...
    private void removeSpeedFreeze(LivingEntity entity) {
        EntityAttributeInstance inst = entity.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED);
        if (inst != null) {
            inst.removeModifier(SPEED_FREEZE_ID);
        }
    }
