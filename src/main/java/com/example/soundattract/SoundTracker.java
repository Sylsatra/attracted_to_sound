...
public class SoundTracker {
    ...
    public static synchronized void addSound(SoundEvent se, BlockPos pos, String dimensionKey, double range, double weight, int lifetime) {
        ResourceLocation soundId = ForgeRegistries.SOUND_EVENTS.getKey(se);
    ...
    private static boolean isBlockInConfigList(BlockState state, Block block, java.util.List<String> configList) {
        ...
        for (String entry : configList) {
            if (entry.startsWith("#")) {
                ...
                TagKey<Block> tagKey = TagKey.create(Registry.BLOCK_REGISTRY, new ResourceLocation(tagName));
    ...
    private static boolean isCustomSolid(BlockState state, Block block) {
        return isBlockInConfigList(state, block, SoundAttractConfig.CUSTOM_SOLID_BLOCKS_CACHE) || state.getMaterial().isSolid();
    }
    private static boolean isCustomNonSolid(BlockState state, Block block) {
        return isBlockInConfigList(state, block, SoundAttractConfig.CUSTOM_NON_SOLID_BLOCKS_CACHE) || !state.getMaterial().isSolid();
    }
