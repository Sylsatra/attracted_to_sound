...
public class SoundTracker {
    ...
    private static boolean isBlockInConfigList(BlockState state, Block block, java.util.List<String> configList) {
        ...
        for (String entry : configList) {
            if (entry.startsWith("#")) {
                ...
                TagKey<Block> tag = TagKey.create(ForgeRegistries.BLOCKS.getRegistryKey(), ResourceLocation.tryParse(tagName));
    ...
    private static boolean isCustomWool(BlockState state, Block block) {
        return isBlockInConfigList(state, block, new ArrayList<>(SoundAttractConfig.customWoolBlocks.get())) || state.is(BlockTags.WOOL);
    }
    private static boolean isCustomSolid(BlockState state, Block block) {
        return isBlockInConfigList(state, block, new ArrayList<>(SoundAttractConfig.customSolidBlocks.get())) || state.isSolidRender(null, null);
    }
    private static boolean isCustomNonSolid(BlockState state, Block block) {
        return isBlockInConfigList(state, block, new ArrayList<>(SoundAttractConfig.customNonSolidBlocks.get())) || !state.isSolidRender(null, null);
    }
    private static boolean isCustomThin(BlockState state, Block block) {
        if (isBlockInConfigList(state, block, new ArrayList<>(SoundAttractConfig.customThinBlocks.get()))) return true;
        ResourceLocation id = ForgeRegistries.BLOCKS.getKey(block);
