    ...
    public static void handle(SoundMessagePayload payload, ServerPlayerEntity sender) {
        try {
            ...
            if (payload.soundId().equals(PointBlankIntegration.PB_GUN_SOUND_ID) && SoundAttractMod.CONFIG.enablePointBlankIntegration) {
                ...
                SoundTracker.addSound(null, pos, serverWorld.getRegistryKey().getValue().toString(), finalRange, finalWeight, lifetime, payload.soundId().toString());
            ...
            if (payload.soundId().equals(SoundMessagePayload.VOICE_CHAT_SOUND_ID)) {
                ...
                if (effectiveRange > 0) {
                    ...
                    SoundTracker.addSound(null, pos, serverWorld.getRegistryKey().getValue().toString(), effectiveRange, payload.weight(), lifetime, SoundMessagePayload.VOICE_CHAT_SOUND_ID.toString());
                }
            } else {
                ...
                SoundTracker.addSound(null, pos, serverWorld.getRegistryKey().getValue().toString(), range, weight, lifetime, payload.soundId().toString());
            }
        } catch (Exception e) {
