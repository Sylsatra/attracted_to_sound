...
public class SoundAttractionEvents {
    ...
    @SubscribeEvent
    public static void onLevelTick(net.minecraftforge.event.TickEvent.WorldTickEvent event) {
        if (event.phase == net.minecraftforge.event.TickEvent.Phase.END && !event.world.isClientSide()) {
            if (event.world instanceof net.minecraft.server.level.ServerLevel serverLevel) {
    ...
    public static void onEntityJoinWorld(EntityJoinWorldEvent event) {
        if (event.getEntity() instanceof net.minecraft.world.entity.Mob mob) {
            ResourceLocation entityId = ForgeRegistries.ENTITIES.getKey(mob.getType());
            if (entityId == null) return;

            String entityIdStr = entityId.toString();
            if (!com.example.soundattract.config.SoundAttractConfig.ATTRACTED_ENTITIES_CACHE.contains(entityIdStr)) {
                return;
            }

            double moveSpeed = com.example.soundattract.config.SoundAttractConfig.COMMON.mobMoveSpeed.get();

            boolean goalExists = mob.goalSelector.getAvailableGoals().stream()
                    .anyMatch(prioritizedGoal -> prioritizedGoal.getGoal() instanceof com.example.soundattract.ai.AttractionGoal);

            if (!goalExists) {
                mob.goalSelector.addGoal(2, new com.example.soundattract.ai.AttractionGoal(mob, moveSpeed));
            }
        }
    }
}
