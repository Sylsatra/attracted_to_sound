...
public class CamoLODUtil {
    ...
    private static boolean isWithinCameraView(net.minecraft.client.Camera camera, LivingEntity target, double horizontalFovDegrees) {
        Vec3 cameraPos = camera.position();
        org.joml.Vector3f forward = camera.getViewRotationMatrix(new org.joml.Matrix4f()).positiveZ(new org.joml.Vector3f()).negate();
        Vec3 lookVector = new Vec3(forward.x(), forward.y(), forward.z());
