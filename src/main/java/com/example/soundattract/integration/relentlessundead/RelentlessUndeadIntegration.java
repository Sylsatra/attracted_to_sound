...
public class RelentlessUndeadIntegration {
    ...
    private static class Inner {
         ...
         private static boolean isMobEligible(Mob mob) {
             try {
                 TagKey<EntityType<?>> hordeTag = TagKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath("forge", "hordes"));
                 return net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.wrapAsHolder(mob.getType()).is(hordeTag);
             } catch (Exception e) {
                 throw new RuntimeException(e);
             }
         }
