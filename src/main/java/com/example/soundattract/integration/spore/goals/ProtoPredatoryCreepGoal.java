...
public class ProtoPredatoryCreepGoal extends Goal {
    ...
    private boolean shouldGrowAt(BlockPos pos) {
        ...
        ResourceLocation id = proto.level().registryAccess().registryOrThrow(Registries.BLOCK).getKey(proto.level().getBlockState(pos).getBlock());
    ...
    private void announceHunt(Proto proto) {
        if (proto.level() instanceof ServerLevel serverLevel) {
            ...
            SoundEvent sound = serverLevel.registryAccess().registryOrThrow(Registries.SOUND_EVENT).get(ResourceLocation.tryParse(soundId));
            if (sound != null) {
