...
public class VanillaIntegrationEvents {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (!player.isSprinting() && !player.isCrouching() && player.getDeltaMovement().horizontalDistanceSqr() > 0.01 && player.isOnGround() && !player.getPose().name().equalsIgnoreCase("SWIMMING")) {
            if (player instanceof ServerPlayer) {
    ...
    private static void sendVanillaSound(String animatorClass, String soundId, double x, double y, double z, ResourceLocation dim, Optional<java.util.UUID> uuid, int range, double weight, double volume, double pitch) {
        ...
        if (isServer) {
            ...
            if (rl.equals(com.example.soundattract.SoundMessage.VOICE_CHAT_SOUND_ID)) {
                if (range > 0) {
