...
public class FovEvents {
    ...
    private static final Set<EntityType<?>> DEVELOPER_EXCLUSIONS = Set.of(
            BuiltInRegistries.ENTITY_TYPE.get(Identifier.withDefaultNamespace("warden")).map(reference -> reference.value()).orElse(null),
            BuiltInRegistries.ENTITY_TYPE.get(Identifier.withDefaultNamespace("ender_dragon")).map(reference -> reference.value()).orElse(null),
            BuiltInRegistries.ENTITY_TYPE.get(Identifier.withDefaultNamespace("wither")).map(reference -> reference.value()).orElse(null)
    );
