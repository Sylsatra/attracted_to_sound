...
public class SoundMessage {
    ...
    public static void handle(SoundMessage msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (sender != null) {
                 serverLevel = sender.server.getLevel(sender.getLevel().dimension());
            } else if (ctx.get().getDirection().getReceptionSide().isServer()) {

                 net.minecraft.server.MinecraftServer server = net.minecraftforge.server.ServerLifecycleHooks.getCurrentServer();
                 if (server != null) {
                     serverLevel = server.getLevel(net.minecraft.resources.ResourceKey.create(net.minecraft.core.Registry.DIMENSION_REGISTRY, msg.dimension));
                 }
            }
