...
public class SoundMessage {
    ...
    public static final ResourceLocation VOICE_CHAT_SOUND_ID =
        ResourceLocation.fromNamespaceAndPath(SoundAttractMod.MOD_ID, "voice_chat");
        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (server != null) {
                try {
                    ...
                    serverLevel = server.getLevel(dimensionKey);
                } catch (Exception e) {
                    SoundAttractMod.LOGGER.error("Error resolving dimension {}", msg.dimension, e);
                }
            } else {
                SoundAttractMod.LOGGER.warn("Could not get server instance to handle SoundMessage.");
            }

            if (serverLevel == null) {
                SoundAttractMod.LOGGER.warn(
                    "Could not get server level (dimension: {}). Server exists: {}",
                    msg.dimension, server != null
                );
                return;
            }
            ...
            if (!serverLevel.dimension().location().equals(msg.dimension)) {
                SoundAttractMod.LOGGER.trace(
                    "SoundMessage dimension mismatch ({} vs {}), ignoring.",
            ...
            BlockPos pos = new BlockPos(
                Mth.floor(msg.x), Mth.floor(msg.y), Mth.floor(msg.z)
            );
            ...
            if (msg.soundId.equals(VOICE_CHAT_SOUND_ID)) {
                if (msg.range > 0) {
                    SoundAttractMod.LOGGER.trace(
                        "Handling voice chat sound at {}, range {}, weight {}",
                        pos, msg.range, msg.weight
                    );
                    SoundTracker.addSound(null, pos, dimString, msg.range, msg.weight, lifetime);
                } else {
                    SoundAttractMod.LOGGER.warn(
                        "Received voice chat sound message with invalid range: {}",
                        msg.range
                    );
                }
            } else {
                ...
                if (snd == null) {
                    SoundAttractMod.LOGGER.warn(
                        "Unknown sound ID: {}", msg.soundId
                    );
                    return;
                }
                if (msg.range >= 0) {
                    SoundAttractMod.LOGGER.trace(
                        "Handling sound {} at {} with client range={}, weight={}",
                        msg.soundId, pos, msg.range, msg.weight
                    );
                    SoundTracker.addSound(snd, pos, dimString, msg.range, msg.weight, lifetime);
                } else {
                    var config = SoundAttractConfig.SOUND_CONFIGS_CACHE.get(snd);
                    if (config != null) {
                        SoundAttractMod.LOGGER.trace(
                            "Handling sound {} at {} using server config range={}, weight={}",
                            msg.soundId, pos, config.range, config.weight
                        );
                        SoundTracker.addSound(snd, pos, dimString, config.range, config.weight, lifetime);
                    } else if (SoundAttractConfig.PLAYER_STEP_SOUNDS_CACHE.contains(snd)) {
                        SoundAttractMod.LOGGER.warn(
                            "Step sound {} defaulted with no config.", msg.soundId
                        );
                    }
                }
            }
